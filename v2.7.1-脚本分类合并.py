...
import os
import traceback
from tkinter import Tk,Frame,Text,EventType,filedialog
...
def windowInit():
    ...
    btn_selectPath.config(command=lambda : [txt_path.delete('1.0','end'),txt_path.insert('0.0',filedialog.askdirectory())])     # 设置点击事件
...
def mergeContent(dir_path,txt_info):
    ...
    for root, dirs, files in os.walk(dir_path):
        ...
        foldername=root[root.replace('/','\\').rfind('\\')+1:]
